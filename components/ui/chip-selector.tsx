...
export default function ChipSelector({
  ...
  useEffect(() => {
    if (searchTerm.trim().length > 0) {
      const results = allOptions.filter(o =>
  ...
  return (
    <div className="space-y-3">
      <div className="flex flex-wrap gap-2">
        {selected
          .filter(s => !featuredOptions.includes(s))
          .map(item => (
            <button
              key={item}
              type="button"
        ...
        <button
          ...
          className="px-3 py-1.5 rounded-full text-sm font-medium border border-dashed border-primary/40 text-primary/70 hover:border-primary hover:text-primary hover:bg-primary/5 transition-all"
        >
          + Other
        </button>
      </div>
